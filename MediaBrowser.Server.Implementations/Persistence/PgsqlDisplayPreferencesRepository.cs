...
{
    /// <summary>
    /// Class PgsqlDisplayPreferencesRepository
    /// </summary>
    public class PgsqlDisplayPreferencesRepository : BasePgsqlRepository, IDisplayPreferencesRepository
    {
        public PgsqlDisplayPreferencesRepository(ILogManager logManager, IJsonSerializer jsonSerializer, IApplicationPaths appPaths, IDbConnector dbConnector)
            ...
            using (var connection = await CreateConnection().ConfigureAwait(false))
                {
                    string[] queries = {
                        ...
                        "CREATE TABLE IF NOT EXISTS userdisplaypreferences (id UUID, userId UUID, client TEXT, data BYTEA, PRIMARY KEY(id, userId, client))",
                        "CREATE INDEX IF NOT EXISTS userdisplaypreferencesindex ON userdisplaypreferences (id, userId, client)"
                    };
                    ...
                    try
                        {
                            ...
                            using (var cmd = connection.CreateCommand())
                                {
                                    cmd.CommandText = "INSERT INTO userdisplaypreferences (id, userId, client, data) VALUES (@1, @2, @3, @4) ON CONFLICT (id, userId, client) DO UPDATE SET data = @4";
