...
{
    public class PgsqlFileOrganizationRepository : BasePgsqlRepository, IFileOrganizationRepository, IDisposable
    {
        private readonly CultureInfo _usCulture = new CultureInfo("en-US");

        public PgsqlFileOrganizationRepository(ILogManager logManager, IServerApplicationPaths appPaths, IDbConnector connector)
            ...
            using (var connection = await CreateConnection().ConfigureAwait(false))
                {
                    string[] queries = {
                        ...
                        "CREATE TABLE IF NOT EXISTS FileOrganizerResults (ResultId UUID PRIMARY KEY, OriginalPath TEXT, TargetPath TEXT, FileLength INT4, OrganizationDate DATE, Status TEXT, OrganizationType TEXT, StatusMessage TEXT, ExtractedName TEXT, ExtractedYear INT4 NULL, ExtractedSeasonNumber INT4 NULL, ExtractedEpisodeNumber INT4 NULL, ExtractedEndingEpisodeNumber INT4 NULL, DuplicatePaths TEXT NULL)",
            ...
            using (var connection = await CreateConnection().ConfigureAwait(false))
                {
                    using (var saveResultCommand = connection.CreateCommand())
                        {
                            saveResultCommand.CommandText = "INSERT INTO FileOrganizerResults (ResultId, OriginalPath, TargetPath, FileLength, OrganizationDate, Status, OrganizationType, StatusMessage, ExtractedName, ExtractedYear, ExtractedSeasonNumber, ExtractedEpisodeNumber, ExtractedEndingEpisodeNumber, DuplicatePaths) VALUES (@ResultId, @OriginalPath, @TargetPath, @FileLength, @OrganizationDate, @Status, @OrganizationType, @StatusMessage, @ExtractedName, @ExtractedYear, @ExtractedSeasonNumber, @ExtractedEpisodeNumber, @ExtractedEndingEpisodeNumber, @DuplicatePaths) ON CONFLICT (ResultId) DO UPDATE SET OriginalPath = @OriginalPath, TargetPath = @TargetPath, FileLength = @FileLength, OrganizationDate = @OrganizationDate, Status = @Status, OrganizationType = @OrganizationType, StatusMessage = @StatusMessage, ExtractedName = @ExtractedName, ExtractedYear = @ExtractedYear, ExtractedSeasonNumber = @ExtractedSeasonNumber, ExtractedEpisodeNumber = @ExtractedEpisodeNumber, ExtractedEndingEpisodeNumber = @ExtractedEndingEpisodeNumber, DuplicatePaths = @DuplicatePaths";
